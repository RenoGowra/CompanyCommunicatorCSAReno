...
import {
    getInitAdaptiveCardPoll, setCardTitlePoll, setCardImageLinkPoll, setCardSummaryPoll,
    setCardAuthorPoll, setCardBtnPoll, setCardPollOptions, setCardPollQuizSelectedValue
...
class NewPoll extends React.Component<INewPollProps, formState> {
    ...
    private getItem = async (id: number) => {
        try {
            ...
            //set card properties
            setCardTitlePoll(this.card, draftMessageDetail.title);
            setCardImageLinkPoll(this.card, draftMessageDetail.imageLink);
            setCardSummaryPoll(this.card, draftMessageDetail.summary);
            setCardAuthorPoll(this.card, draftMessageDetail.author);
    ...
    public setDefaultCard = (card: any) => {
        ...
        setCardTitlePoll(card, titleAsString);
        let imgUrl = getBaseUrl() + "/image/imagePlaceholder.png";
        setCardImageLinkPoll(card, imgUrl);
        setCardSummaryPoll(card, summaryAsString);
        setCardAuthorPoll(card, authorAsString);
        setCardBtnPoll(card, this.localize("PollSubmitVote"), "https://adaptivecards.io");
    ...
    //Added: changed hard core restriction from 1MB to 2MB
    ...
    //Added: taken case if imageUploadBlobStorage is false,store max 32KB images in azure tables
        ...
        if (file) {
            ...
            if (this.imageUploadBlobStorage) {
                ...
                fileReader.onload = () => {
                    ...
                    setCardImageLinkPoll(this.card, resizedImageAsBase64);
                    this.updateCard();
                    this.setState({
                ...
                Resizer.imageFileResizer(file, 400, 400, 'JPEG', 80, 0,
                    uri => {
                        if (uri.toString().length < maxCardSize - cardsize) {
                            ...
                            setCardImageLinkPoll(this.card, uri.toString());
                            this.updateCard();
    ...
    //Added: handleUploadClick for image upload
    ...
    //Added: handleUploadClick for csv file upload
    //Function calling a click event on a hidden file input
    ...
    private onItemChecked(i: number, checked: boolean) {
        ...
        if (answers && checked) {
            ...
            setCardTitlePoll(this.card, this.state.title);
            setCardImageLinkPoll(this.card, this.state.imageLink);
            setCardSummaryPoll(this.card, this.state.summary);
            setCardAuthorPoll(this.card, this.state.author);
            setCardPollOptions(this.card, this.state.isPollMultipleChoice, this.state.pollOptions);
                        ...
                        <Flex column className="formContainer" vAlign="stretch" gap="gap.small">
                            ...
                            <Flex className="footerContainer" vAlign="end" hAlign="end">
                                <Flex className="buttonContainer" gap="gap.small">
                                    ...
                                    <Button content={this.localize("SaveAsDraft")} disabled={this.isSaveBtnDisabled() || this.state.selectedSchedule} id="saveBtn" onClick={this.onSave} primary={!this.state.selectedSchedule} />
                                </Flex>
                            </Flex>
                        </Flex>
                    </div>
    ...
    private onTitleChanged = (event: any) => {
        ...
        setCardTitlePoll(this.card, event.target.value);
        setCardImageLinkPoll(this.card, this.state.imageLink);
        setCardSummaryPoll(this.card, this.state.summary);
        setCardAuthorPoll(this.card, this.state.author);
        this.setState({
    ...
    private onImageLinkChanged = (event: any) => {
        ...
        setCardTitlePoll(this.card, this.state.title);
        setCardImageLinkPoll(this.card, event.target.value);
        setCardSummaryPoll(this.card, this.state.summary);
        setCardAuthorPoll(this.card, this.state.author);
        this.setState({
    ...
    private onSummaryChanged = (event: any) => {
        ...
        setCardTitlePoll(this.card, this.state.title);
        setCardImageLinkPoll(this.card, this.state.imageLink);
        setCardSummaryPoll(this.card, event.target.value);
        setCardAuthorPoll(this.card, this.state.author);
        this.setState({
    ...
    private onAuthorChanged = (event: any) => {
        ...
        setCardImageLinkPoll(this.card, this.state.imageLink);
        setCardSummaryPoll(this.card, this.state.summary);
        setCardAuthorPoll(this.card, event.target.value);
